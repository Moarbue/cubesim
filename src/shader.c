...
int shader_new(unsigned int *shader, const char *vertex_path, const char *fragment_path, const char *geometry_path) {
    ...
    if (vert_in == NULL) {
        fprintf(stderr, "Shader Error: Failed to open vertex_path. Provided path was: %s\n", vertex_path);
        return 1;
    }
    ...
    switch (ret) {
        ...
        case READ_FILE_ERROR:
            fprintf(stderr, "Shader Error: A read error occured while reading from vertex shader file %s\n", vertex_path);
            return 1;
        case READ_FILE_TOOMUCH:
            fprintf(stderr, "Shader Error: Vertex shader file %s is too big\n", vertex_path);
            return 1;
        case READ_FILE_NOMEM:
            fprintf(stderr, "Shader Error: Ran out of memory while reading from vertex shader file %s\n", vertex_path);
            return 1;
    }
    ...
    // Check if file stream was openend correctly
    if (frag_in == NULL) {
        fprintf(stderr, "Shader Error: Failed to open fragment_path. Provided path was: %s\n", fragment_path);
        return 1;
    }
    ...
    switch (ret) {
        case READ_FILE_OK:
            ...
            break;
        case READ_FILE_ERROR:
            fprintf(stderr, "Shader Error: A read error occured while reading from fragment shader file %s\n", fragment_path);
            return 1;
        case READ_FILE_TOOMUCH:
            fprintf(stderr, "Shader Error: Fragment shader file %s is too big\n", fragment_path);
            return 1;
        case READ_FILE_NOMEM:
            fprintf(stderr, "Shader Error: Ran out of memory while reading from fragment shader file %s\n", fragment_path);
            return 1;
    }
    ...
    if (!success) {
        fprintf(stderr, "Shader Error: Failed to compile vertex shader. Provided file: %s\n", vertex_path);
        return 1;
    }
    fprintf(stdout, "Created vertex shader.\n");
    ...
    if (!success) {
        fprintf(stderr, "Shader Error: Failed to compile fragment shader. Provided file: %s\n", fragment_path);
        return 1;
    }
    ...
    if (!success) {
        fprintf(stderr, "Shader Error: Failed to link shader program.\nNote: Check shader sources for errors!\n");
        return 1;
    }
    fprintf(stdout, "Created shader program.\n");
